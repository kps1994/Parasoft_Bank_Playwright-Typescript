...
import { formatAPIRequest } from "../../fixture/APIHelper.ts";
...
test("Create POST API Request using dynamic api request body in playwright & typescript", async ({
  ...
  const filePath = path.join(
    __dirname,
    "../../test_data/api_requests/Dynamic_POST_API_Request.json",
  );
...
test("Create POST API Request using dynamic api request body in playwright & typescript 2", async ({
  request,
}) => {
  // Reading json file
  const filePath = path.join(
